...
public class ConvertDateToBinarySolution
{
    public string ConvertDateToBinary( string date )
    {
        ...
        string yearB = GetBinaryString( dateParts[0] );
        string monthB = GetBinaryString( dateParts[1] );
        string dayB = GetBinaryString( dateParts[2] );

        return $"{yearB}-{monthB}-{dayB}";
    }

    private static string GetBinaryString( string datePart )
    {
        return Convert.ToString( int.Parse( datePart ), 2 );
    }
}
