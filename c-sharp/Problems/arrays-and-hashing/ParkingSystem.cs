...
{
    private readonly Dictionary<int, int> _carSizeToSpaceAmount;

    public ParkingSystem( int big, int medium, int small )
    {
        this._carSizeToSpaceAmount = new()
        {
            { 1, big    },
            { 2, medium },
            { 3, small  }
    ...
    {
        ref int spaceLeftForCarType = ref CollectionsMarshal.GetValueRefOrNullRef( this._carSizeToSpaceAmount, carType );

        if ( spaceLeftForCarType > 0 )
        {
            spaceLeftForCarType--;
