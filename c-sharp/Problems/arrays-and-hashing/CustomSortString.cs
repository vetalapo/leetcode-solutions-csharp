 ...
 *     from s should be arranged as "b", "c", "a". "d" can be placed at any position since it's not in order.
...
public class CustomSortStringSolution
{
    public string CustomSortString( string order, string s )
    {
        ...
        foreach ( char c in s )
        {
            if ( sMap.TryGetValue( c, out int value ) )
            {
                sMap[c] = ++value;
            }
            else
        ...
        {
            if ( sMap.Remove( c, out int amount ) )
            {
                result.Add( new String( c, amount ) );
            }
        }

        foreach ( (char key, int value) in sMap )
        {
